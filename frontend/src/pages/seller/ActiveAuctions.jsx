  ...
  useEffect(() => {
    const fetchAuctions = async () => {
      try {
        ...
        setAuctions((response.data || []).filter((auction) => auction.status === "ACTIVE"));
      } catch (err) {
        console.error(err);
        setError("Unable to load your active auctions.");
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-gray-600">Monitor your currently running auctions.</p>
          </div>
        ...
        {loading ? (
          <p className="text-gray-500">Loading active auctions...</p>
        ) : auctions.length === 0 ? (
          <div className="rounded-3xl border border-gray-200 bg-gray-50 p-8 shadow-sm">No active auctions right now.</div>
        ) : (
